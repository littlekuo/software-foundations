...
Proof.
  ...
  rewrite -> H2 in H12. injection H12 as H13.
  transitivity z.
  apply H11. symmetry. apply H13.
Qed. 
...
Proof.
  intros n. induction n as [|n' IHn'].
  - intros m eq. symmetry. apply eqb_0_l. rewrite -> eq. reflexivity.
  - intros m eq. destruct m as [|m'].
    + discriminate eq.
    + apply f_equal. apply IHn'. simpl in eq. rewrite -> eq. reflexivity.
Qed.
...
Proof.
  ...
  - intros m eq. destruct m as [|m'].
    ...
    apply S_injective in eq. rewrite -> eq. reflexivity.
Qed.
(** [] *)
...
Proof.
 ...
 - intros n H. simpl in H.  destruct n as [|n'].
   + discriminate H.
   + injection H as H1. simpl.  apply IHl' in H1. 
...
(** **** Exercise: 3 stars, advanced, optional (eqb_sym_informal)
   ...
   Proof: *)
   (* Let [n] be a [nat]. We prove by induction on [n] that, for
      any [n], [(n =? m) = (m =? n)].
...
Proof.
  ...
  assert (H3 :n = p).
  { transitivity (m). rewrite -> H1. reflexivity. rewrite -> H2. reflexivity. } 
  rewrite -> H3. rewrite -> eqb_refl. reflexivity.
Qed.
...
Proof.
  ...
  - intros lf H. simpl in H. discriminate.
  - intros lfs H. destruct (test s) eqn:Heq1.
    + simpl in H. rewrite -> Heq1 in H. injection H as H1 H2.
      rewrite <- H1. rewrite -> Heq1. reflexivity.
    + simpl in H. rewrite -> Heq1 in H. apply IHl' with (lf:=lfs). 
...
Proof. 
  ...
  - destruct (test fst) eqn:Eq'.
    { simpl. rewrite -> Eq'.  unfold existsb'. simpl. rewrite -> Eq'. simpl.  
     reflexivity. }
    { simpl.  unfold existsb'. simpl.  rewrite -> Eq'. simpl.
