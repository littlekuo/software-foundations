...
(**   unfold not in ~~P. we get [(P -> False) -> False].
     so in the conditions of [P] and [P->False], we need prove False.
     
     we just apply [P->False] in [P]. Qed. *)  
...
(** unfold not. we need prove [forall P:Prop, (P /\ (P -> False)) -> False]
...
Proof.
   unfold not.
   intros X P H [xo Hx]. apply Hx. apply H.
  Qed.
(** [] *)
...
Proof.
   intros X P Q. split.
   - intros [xo [H1 | H2]]. 
     {  left. exists xo.  apply H1. }
     {  right. exists xo. apply H2. }
   - intros [[x1 H1] | [x2 H2]].
...
Proof.
  ...
  - simpl. intros [H1 | H2].
    + left. left.  apply H1.
    + apply or_assoc. right. apply IHt in H2.  apply H2.
  - induction l as [|h t].
    { simpl. intros [H1 | H2].
      + destruct H1.
      + apply H2.
...
Proof.
  ...
  - induction l as [|h t].
    ...
    { simpl .  intros H. split.
      ...
      { apply IHt. intros x H1. apply H. right. apply H1. }
    }
  - induction l as [|h t].
    { simpl.  intros H x H2. destruct H2. }
...
Proof.
   ...
   induction l1 as [|t l'].
   - reflexivity. 
   - intros H. simpl.  rewrite -> IHl'.  rewrite <- app_assoc. reflexivity.
Qed.
...
Proof.
  ...
  - destruct IHn' as [k0 IHk0].
    destruct (evenb n') eqn:Eq'.
    { exists k0.  rewrite -> evenb_S. rewrite -> Eq'. simpl.  rewrite -> IHk0.  reflexivity. }
    { exists (1 + k0). rewrite -> evenb_S. rewrite -> Eq'. simpl. rewrite -> IHk0.  reflexivity. }
 Qed.
(** [] *)
...
Proof.
  intros. split.
  - intros H. assert (H':b2 = true).
    { apply andb_true_elim2 with (b:=b1). apply H. }
    rewrite -> H' in H. rewrite andb_commutative in H. simpl in H. split.
    { apply H. }  { apply H'. }
...
Theorem eqb_neq : forall x y : nat,
  x =? y = false <-> x <> y.
Proof.
  intros. rewrite <- not_true_iff_false. unfold not. rewrite -> eqb_eq. 
  reflexivity. 
 Qed.
(** [] *)
...
Fixpoint eqb_list {A : Type} (eqb : A -> A -> bool)
    ...
    | x :: l1' => match l2 with
                  | [] => false
                  | y :: l2' => (andb (eqb x y) (eqb_list eqb l1' l2'))
                  end
    end.
...
Proof.
  ...
  - generalize dependent l2. induction l1 as [|x t IHt'].
    { intros l2 H1. destruct l2 eqn:Eq'.
     { reflexivity. }
     { simpl in H1. discriminate H1. }
    } 
    { intro l2. destruct l2 eqn:Eq'.
     { intros H1. discriminate H1. }
     { simpl.  intros H1. rewrite -> andb_true_iff in H1. 
       destruct H1 as [H11 H12].
      apply IHt' in H12. apply H in H11.  rewrite -> H11, H12. reflexivity. }
    }
  - generalize dependent l2. induction l1 as [|x t IHt'].
    { intros l2 H1 .  rewrite <- H1. reflexivity. }
    { intros l2 H1.  rewrite <- H1. simpl. rewrite -> andb_true_iff. 
      split.
      { apply H. reflexivity. }
...
Proof.
  ...
  - induction l as [| x l' IHl'].
    ...
    { simpl.  intros H. rewrite -> andb_true_iff in H. destruct H as [H1 H2].
      split.
      { apply H1. }
      { apply IHl'. apply H2. }
    }
...
Proof.
  ...
  - apply H1.
  - unfold not in H1, H0. assert (H2 : False).
...
Lemma from1to2 : excluded_middle -> peirce.
Proof.
  intros H1. unfold peirce. intros P Q. destruct H1 with (P := P).
  - intros H2.  apply H. 
  - intros H2. apply H2. apply not_implies_our_not. apply H.
 Qed. 


Lemma from2to3 : peirce -> double_negation_elimination.
Proof.
  intros H.  unfold double_negation_elimination.
  intros P. unfold not. intros H1. apply H with (Q := False). intros H2. apply H1 in H2.
  destruct H2.
Qed.

Lemma from3to4 : double_negation_elimination ->  de_morgan_not_and_not.
Proof.
 intros H. unfold de_morgan_not_and_not. unfold not.
 intros P Q H1.  apply H. unfold not. 
 intros H2.  assert (H21 : P -> False).
 { intros H'.  apply H2. left. apply H'. }
 assert (H22 : Q -> False ).
 { intros H'.  apply H2. right. apply H'. }
 apply H1. split.
 apply H21. apply H22.
Qed.


Lemma from4to5 : de_morgan_not_and_not -> implies_to_or.
Proof. 
  intros H. unfold implies_to_or.
  intros P Q.
  intros H1.
  apply H.
  unfold not. intros [H21 H22]. apply H21. intros H23.
  apply H22. apply H1. apply H23.
Qed.

Lemma from5to1 : implies_to_or -> excluded_middle.
Proof.
   intros H. unfold excluded_middle. 
   intros P. apply or_commut. apply H.  intros H1. apply H1.
Qed.

(** the below is trival*)
Theorem all_equivalent:
...
Proof.
  ...
  split.  
  { apply from1to2. }  
  { intros H.  apply from5to1, from4to5, from3to4, from2to3. apply H. }
  split.
  split.  
  {  intros H.  apply from2to3, from1to2. apply H. }
  {  intros H.  apply from5to1, from4to5, from3to4. apply H. }
  split.
  split.
  { intros H. apply from3to4, from2to3, from1to2. apply H. }
  { intros H. apply from5to1, from4to5. apply H. }
  split.
  split.
  { intros H. apply from4to5, from3to4, from2to3, from1to2. apply H. }
  { apply from5to1. }
  split.
  split.
  { apply from2to3. }
  { intros H.  apply from1to2, from5to1, from4to5, from3to4. apply H. }
  split.
  split.
  { intros H. apply from3to4, from2to3.  apply H. }
  { intros H. apply from1to2, from5to1, from4to5.  apply H. }
  split.
  split.
  { intros H. apply from4to5, from3to4, from2to3. apply H. }
  { intros H. apply from1to2, from5to1.  apply H. }
  split.
  split.
  { apply from3to4. } 
  { intros H. apply from2to3, from1to2, from5to1, from4to5.  apply H. }
  split.
  split.
  { intros H. apply from4to5, from3to4. apply H. }
  { intros H. apply from2to3, from1to2, from5to1.  apply H. }
  split.
  { apply from4to5. }
  { intros H. apply from3to4, from2to3, from1to2, from5to1. apply H. }
Qed.
