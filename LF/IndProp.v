...
Theorem even5_nonsense :
  even 5 -> 2 + 2 = 9.
Proof.
  intros H. simpl. inversion H. inversion H1. apply one_not_even in H3.
  destruct H3.
Qed.
(** [] *)
...
Proof.
 ...
 {
  intros H.
  induction H as [| |n' m' E E' IH IE].
  + apply ev_0.
  + apply ev_SS. apply ev_0.
  + apply ev_sum. apply E'. apply IE.
 }
 {
  intros H.
...
Proof.
  intros n m p H1 H2.
  assert (H: even (double n)). {
    apply ev_double.
  }
  apply ev_ev__ev with (n := (double n)). 
  - assert (H': (double n + (m + p)) = (n + m + (n + p))).
    {
       rewrite -> double_plus. rewrite -> plus_assoc, plus_assoc.
       rewrite <- plus_assoc with (n := n) (m := n) (p := m).
       rewrite -> plus_comm with (n := n) (m := (n+m)). reflexivity.  
    }
    rewrite -> H'.  apply ev_sum. apply H1. apply H2.
  - apply H. Qed.
...
Proof.
  ...
  induction H1 as [|n' E IH].
  - apply H2.
  - apply IH. apply  Sn_le_m__n_le_m. apply H2.
Qed.  
...
Proof.
  intros n m H.
  apply plus_lt with (n1 := 0) (n2 := n) (m := S m).
  simpl. unfold lt in H. unfold lt. apply le_S. apply H.
Qed.
...
Lemma R_n_0_n:  forall n, R n 0 n.
Proof.
  intros n.
  induction n.
  - apply c1.
...
Proof.
  split.
  {
    ...
    induction H.
    - reflexivity.
    - unfold fR. simpl. unfold fR in IHR. rewrite -> IHR. reflexivity.  
    ...
    - unfold fR in IHR. unfold fR. rewrite -> plus_comm in IHR. apply IHR.
  }
  {
    destruct m eqn:E'.
    - intros H1. unfold fR in H1. simpl in H1. rewrite -> H1. apply R_0_n_n.
    - assert (H: R (S n0) n (S n0 + n)). {
           induction n.
           - rewrite -> plus_comm. simpl. apply R_n_0_n.
           - simpl. rewrite <- plus_n_Sm. apply c3. apply IHn. 
     }
     intros H1. unfold fR in H1. rewrite <- H1. apply H.
  }
Qed.
...
Inductive subseq : list nat -> list nat -> Prop :=
  | sub0 (l: list nat): subseq nil l
  | sub1 (l1 : list nat) (l2 : list nat) (a : nat) (H: subseq l1 l2) : (subseq l1 (a :: l2))
  | sub2 (l1 : list nat) (l2 : list nat) (a : nat) (H: subseq l1 l2) : (subseq (a :: l1) (a :: l2))     
.
...
Proof.
  ...
  induction H.
  - apply sub0. 
  - assert (H1: (a :: l2) ++ l3 = a :: (l2 ++ l3)).
    {
      unfold app. reflexivity.
    }
    rewrite -> H1.  apply sub1. apply IHsubseq.
  - assert (H1: (a :: l2) ++ l3 = a :: (l2 ++ l3)).
    {
      unfold app. reflexivity.
    }
    rewrite -> H1. apply sub2. apply IHsubseq.
Qed.
...
Proof.
  ...
  - intros. inversion T12. 
    + apply sub0.
  - intros. apply sub1. apply IHT23. apply T12.
  - intros. inversion T12.
    + apply sub0.
    + apply sub1. apply IHT23. apply H1.
    + apply sub2. apply IHT23. apply H1.
Qed. 
...
Proof.
  ...
  - intros H. simpl. apply MStarApp.  
    ...
    + apply IHss.  intros s H'.  apply H. unfold In. right. unfold In in H'.
     apply H'. Qed. 
...
Proof.
  ...
  - generalize dependent s1.
    induction s2. 
    {  simpl. intros s1. intros H.
       inversion H. reflexivity. }
    { intros s1. simpl. intros H. 
      inversion H.  inversion H3. apply IHs2 in H4.  rewrite -> H4.
      reflexivity. }
  - generalize dependent s1.
    induction s2.
    { intros s1 H. simpl. rewrite -> H. apply MEmpty . }
    { intros. simpl. assert (H': x :: s2 = [x] ++ s2). { reflexivity. }
      rewrite -> H , H'. apply  MApp. 
      {  apply MChar. }
