...
Theorem silly_ex :
     (forall n, evenb n = true -> oddb (S n) = true) -> oddb 3 = true -> evenb 4 = true.
Proof.
  intros eq1 eq2.
  apply eq2.
  Qed.
...
Proof.
  ...
  injection H2 as H21 H22. symmetry. apply H21. Qed.
...
Proof.
  ...
  - intros m H1. destruct m as [|m'] eqn:Em'.
     { simpl in H1. apply H1.  }
     { simpl in H1. discriminate H1.  } 
  - intros m H2. destruct m as [|m'] eqn:Em'.
     {  simpl in H2. discriminate H2. }
     {  apply f_equal. apply IHn'. 
        rewrite <- plus_n_Sm, <- plus_n_Sm in H2. 
        injection H2 as H21. apply H21. }
 Qed. 
...
Proof.
  ...
  - intros m eq. destruct m as [| m'] eqn:Eq.
    { reflexivity. }
    {  discriminate eq. }
  - intros m eq. destruct m as [| m'] eqn:Eq.
    {  discriminate eq. } 
    {  simpl in eq. apply f_equal. apply IHn'. simpl in eq. apply eq. }
 Qed. 
...
Proof:
       by induction on n.
      - First, suppose n = O, we must show 
          forall m, O =? m = true -> O = m.
        When m = O , it is true.
        When m = S m', the assumption is nonsensical, so it is true. 
      - Second, suppose n = S n', where 
          forall m, n' =? m = true ->  n' = m.
        We must show
          forall m, n =? m = true -> n = m.
        when m = O, it becomes S n' =? O = true -> S n' = O. the assumption is nonsensical, so it is true.
        when m = S m', it becomes (S n' =? S m') = true -> S n' = S m'.
              simplify, in the condition of n' =? m' = true , 
                  we need prove S n' = S m'.
            by induction hypothesis, we get (n' = ? m') = true -> n' = m' (let m is m').
          so in the condition of  n' =? m' = true, we can get n' = m', 
                 So S n' = S m'.
       Qed. 
*)
...
Proof.
  intros n X l.
  generalize dependent n.
  induction l as [| l' re' IHl'].
  - intros n eq. destruct n as [| n'] eqn:Eq.
    { reflexivity. }
    { reflexivity. }
  - intros n eq. destruct n as [| n'] eqn:Eq.
    { discriminate eq. }
    { simpl. simpl in eq. apply S_injective in eq.  apply IHl'.  apply eq. }
   Qed.
...
Proof.
  intros X Y l.
  induction l as [|l' re IHl'].
  - intros l1 l2 eq.  unfold split in eq. injection eq as eq1 eq2.  rewrite <- eq1.  reflexivity.
  - intros l1 l2 eq.  destruct l' eqn: c1. destruct (split re)  eqn:c2.
     unfold split in c2. unfold split in eq. rewrite -> c2 in eq. injection eq as eq5 eq6. 
     rewrite <- eq5.  rewrite <- eq6. simpl.  rewrite -> IHl'. 
        { reflexivity. }
        { reflexivity. }
   Qed. 
...
Proof.
  intros f b.
  destruct b eqn:Eqb.
  { destruct (f true) eqn:Eqf1.
    { rewrite Eqf1. apply Eqf1.  }
    { destruct (f false) eqn:Eqf2.
       { apply Eqf1. }
       { apply Eqf2. } 
    }
   }
   { destruct (f false) eqn:Eqf1.
     { destruct (f true) eqn:Eqf2.
       { apply Eqf2. }
       { apply Eqf1. } 
     }
     { rewrite Eqf1. apply Eqf1.  }
...
(** **** Exercise: 3 stars, advanced, optional (eqb_sym_informal)  
   ...
   Proof: Let [n] be a [nat]. We prove by induction on [n] that, for
      any [n], [(n =? m) = (m =? n)].

      - First, suppose [n = 0], There are two cases to consider for [m].
        If [m = 0], we need prove [true = true] which is obvious. 
        Otherwise, if [m = S m'], by the definition of [=?],
        we need prove [false = false] which is obvious.

      - Second, suppose [n = S n'] and that [m] is again a number,
        We must show that [(n =? m) = (m =? n)], with the induction 
        hypothesis that for every number [s], [(n' =? s) = (s =? n')]

        There are two cases to consider for [n].

        If [m = 0], simplify,  we need prove [false = false] which is obvious.

        if [m = S m'], simplify, we must prove [(n' =? m') = (m' =? n')]
        Instantiating the induction hypothesis with [m'] thus
        allows us to conclude that [(n' =? m') = (m' =? n')].
...
Proof.
  ...
  rewrite <- H1 in H2.
  assert (H: true = (n =? n)). { apply eqb_refl. }
...
Proof.
  intros X Y l1.
  induction l1 as [|l' re IHl'].
  - intros l2 eq.  simpl in eq. destruct l2 eqn:Eql.
    { reflexivity. }
    { discriminate eq. }
  - intros l2 eq.  destruct l2 eqn:eq2.
    { simpl in eq.  discriminate. }
    { simpl in eq. injection eq as eq'. apply IHl' in eq'.  simpl.  rewrite -> eq'.  
        reflexivity.
    }
   Qed. 
...
Proof.
  ...
  induction l as [|fst re IHl'].
  - intro H.  discriminate H.
  - intros H.  destruct (test fst) eqn:Eqr.
    {  simpl in H.  rewrite -> Eqr in H.  injection H as H1 H2.  rewrite -> H1 in Eqr. apply Eqr. }
    {  simpl in H.  rewrite -> Eqr in H. apply IHl' in H. apply H. }
  Qed.
(** [] *)
...
Proof. 
   ...
   induction l as [|fst re IHl'].
   - reflexivity.
   - destruct (test fst) eqn:Eq'.
     { simpl.  unfold existsb'.  unfold forallb. rewrite -> Eq' . reflexivity. }
     { simpl.  unfold existsb'.  unfold forallb. rewrite -> Eq'. simpl. 
      unfold existsb' in IHl'.  unfold forallb in IHl'. apply IHl'. }
  Qed.
(** [] *)
