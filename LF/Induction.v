...
Proof.
  intros n m p q.
  assert (* We just need to swap (n + m) for (m + n)... seems
...
(** **** Exercise: 2 stars, advanced, recommended (plus_comm_informal)  
    ...
    - First, suppose [n = 0].  We must show
      ...
      This follows directly from the definition of [+] and plus_n_O.
    ...
    - Next, suppose [n = S n'], where
      ...
      We must show
        ...
        left = (S n') + m = S (n' + m) (by definition of [+])
                          = S (m + n') (by induction)
                          = m + S n'   (by plus_n_Sm)
                          = right
*)
...
(** **** Exercise: 2 stars, standard, optional (eqb_refl_informal)  
    ...
    - First, when n = 0, 0 =? 0 = true follows directly from the definition of [=?, eqb].
    
    - Second, suppose [n = S n'],  where n' =? n' = true.
      
      we must show (S n') =? (S n')  = true.
      
      left = (n' =? n') (by the definition of [=?, eqb].
           = true.  (by induction)
...
Proof.
   ...
   assert (H1: m + n = n + m).
   { rewrite -> plus_comm. reflexivity. }
   rewrite -> plus_assoc. rewrite <- H1.  rewrite -> plus_assoc. reflexivity. Qed.
...
Proof.
   ...
   - simpl. rewrite -> IHm'.  rewrite -> mult_n_Sm. reflexivity. Qed.
(** [] *)
...
Proof.
   intros m. induction m as [|m' IHm'|t IHt'].
   - simpl. reflexivity.
   - simpl. reflexivity.
   - simpl. rewrite <- plus_n_O. rewrite <- plus_n_O. rewrite IHt'. rewrite -> plus_swap. 
     rewrite -> plus_assoc. rewrite -> plus_assoc. reflexivity. Qed.
...
Proof.
  intros n. induction n as [|n' IHn'].
  - simpl. reflexivity.
  - simpl. rewrite -> bin_to_nat_pres_incr.  rewrite -> IHn'. rewrite -> plus_comm. reflexivity.  Qed.
...
(** Because bin_to_nat is not injective, for example, bin_to_nat(A Z) and 
    bin_to_nat(Z) are the same O *)
...
Lemma helper1: forall b, incr (incr (bindouble b)) = bindouble (incr b).
Proof. 
  intros b. induction b as [|b' IHb'|t IHt'].
  - simpl. reflexivity.
  - simpl. reflexivity.
  - simpl. reflexivity. Qed.
...
Lemma helper2: forall n, nat_to_bin (n + n) = bindouble (nat_to_bin n).
Proof. 
  intros n. induction n as [|n' IHn'].
  -  simpl.  reflexivity. 
  -  simpl.  rewrite <- plus_n_Sm. simpl. rewrite -> IHn'. rewrite -> helper1. reflexivity. Qed.


Lemma helper3: forall n, nat_to_bin (n + n + 1) = B (nat_to_bin n).
Proof.
  intros n. induction n as [|n' IHn'].
  - simpl. reflexivity. 
  - simpl. rewrite <- plus_n_Sm with (m:=n').  rewrite <- plus_Sn_m. simpl. rewrite -> IHn'.
    simpl. reflexivity.
  Qed.



Theorem normalize_eqb: forall b:bin, nat_to_bin (bin_to_nat b) = normalize b.
Proof. 
  intros b. induction b as [|b' IHb'|t IHt'].
  - simpl. reflexivity.
  - simpl. rewrite <- plus_n_O. rewrite -> helper2.  rewrite -> IHb'.  unfold bindouble.  reflexivity.
  - simpl. rewrite <- plus_n_O. rewrite -> helper3.  rewrite -> IHt'.  reflexivity. Qed.
