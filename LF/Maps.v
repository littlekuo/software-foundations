...
Proof.
   ...
   intros. destruct (eqb_string x x0) eqn:Eqr.
   - unfold t_update. rewrite -> Eqr. trivial.
   - unfold t_update. rewrite -> Eqr. trivial.
Qed.
...
Proof.
   intros x y. apply iff_reflect. symmetry. apply eqb_string_true_iff. Qed.
...
Proof.
   intros. apply functional_extensionality.
   intros. destruct (eqb_string x x0) eqn:Eqr.
   - unfold t_update. rewrite -> Eqr. apply eqb_string_true_iff in Eqr. 
     rewrite -> Eqr. trivial. 
   - unfold t_update. rewrite -> Eqr. trivial.
Qed.
(** [] *)
...
Proof.
  intros.  apply functional_extensionality.
  intros.  destruct (eqb_string x1 x) eqn:Eqr.
  -  destruct (eqb_string x2 x) eqn:Eqt.
     + apply eqb_string_true_iff in Eqr. apply eqb_string_true_iff in Eqt. 
       rewrite <- Eqr in Eqt.  apply H in Eqt. destruct Eqt.
     + unfold t_update. rewrite -> Eqr. rewrite -> Eqt.  trivial.
  - destruct (eqb_string x2 x) eqn:Eqt.
     + unfold t_update. rewrite -> Eqt.  rewrite -> Eqr. trivial.
     + unfold t_update. rewrite -> Eqt.  rewrite -> Eqr. trivial.
Qed.
