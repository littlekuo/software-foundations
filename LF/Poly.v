...
Proof.
  ...
  - simpl.  rewrite -> IHl', app_assoc. reflexivity. Qed.
...
Fixpoint split {X Y : Type} (l : list (X*Y))
   ...
   match l with
   | [] => ([], [])
   | t :: d => match (split d) with
               |x => (cons (fst t) (fst x), cons (snd t) (snd x))
               end
   end.
...
Proof.
  ...
  - simpl. reflexivity. 
  - simpl. rewrite -> map_app, IHl'. reflexivity. Qed.
(** [] *)
...
Example fold_example4 :
  fold (fun x y => length x + y) [[1];[];[2;3];[4]] 0 = 4.
Proof. simpl.  reflexivity. Qed.
...
Proof.
  intros X Y Z f p.
  simpl. unfold prod_uncurry, prod_curry. destruct p eqn:Ep'.
  reflexivity. Qed.
(** [] *)
...
(**
  Proof:
      ...
      - First, suppose l = [], we must show 
        ...
        This follows directly from the definition of nth_error.
      - Second, suppose l = t :: d, where 
             forall n, length d = n -> @nth_error X d n = None.
        We must show
             forall n, length l = n -> @nth_error X l n = None.
        
        if length l = n, then 1 + length d = n       
        So @nth_error X (t :: d) n = @nth_error X d (pred n) (by  definition of nth_error.)
                                   = @nth_error s d (length d) 
                                   = None. (by induction)
       Qed. 
*)

Lemma nth_error_fomal: forall (X : Type) (l : list X)  (n : nat) , length l = n  -> @nth_error X l n = None.
Proof.
   intros X l n. 
   generalize dependent n.
   induction l as [|t l' IHl'].
   - simpl. reflexivity.
   - simpl. intros n H. rewrite <- H. simpl. apply IHl'. reflexivity. Qed.
...
Definition cnat_to_nat (n : cnat) : nat := n nat S O.

Fixpoint nat_to_cnat (m : nat) : cnat := 
   match m with 
   | O => zero
   | S m' => succ (nat_to_cnat m')
  end.

Definition mult' (n m : cnat) : cnat := nat_to_cnat (mult (cnat_to_nat n) (cnat_to_nat m)).
...
Definition exp' (n m : cnat) : cnat := nat_to_cnat (exp (cnat_to_nat n) (cnat_to_nat m)).
